...
signal toggled(is_shown: bool)

const THEME_DEFAULT := "uid://dq4nntds66bix"

const BuiltinCommands := preload("uid://npao6ckwv7cm")
const CommandEntry := preload("uid://ddtreeatktyov")
const ConfigMapper := preload("uid://dvokj0q23nb50")
const ConsoleOptions := preload("uid://c7a12a1pe5esr")
const Util := preload("uid://cw6s1es6yjip5")
const CommandHistory := preload("uid://dc55ouwu3ylf")
const HistoryGui := preload("uid://cpl5jb0mwxanh")
...
var _argument_autocomplete_sources: Dictionary = {}
...
func _handle_history_input(p_event: InputEvent) -> void:
	# Allow tab complete (reverse)
	if p_event.is_action_pressed("developer_console_auto_complete_reverse"):
		_reverse_autocomplete()
		get_viewport().set_input_as_handled()
...
func register_command(p_func: Callable, p_name: String = "", p_desc: String = "") -> void:
	if p_name and not Util.is_valid_command_sequence(p_name):
		push_error("Console: Failed to register command: %s. Name can have up to 4 space-separated identifiers, using letters, digits, or underscores, starting with non-digit." % [p_name])
		return

	if not _validate_callable(p_func):
		push_error("Console: Failed to register command: %s" % p_name)
		return
	var _name: String = p_name
	if _name.is_empty():
		if p_func.is_custom():
			push_error("Console: Failed to register command: Callable is not method and no _name was provided")
			return
		_name = p_func.get_method().trim_prefix("_").trim_prefix("cmd_")
	if not OS.is_debug_build() and _options.commands_disabled_in_release.has(_name):
		return
	if _commands.has(_name):
		push_error("Console: Command already registered: " + p_name)
		return
	# Note: It should be possible to have an alias with the same _name.
	_commands[_name] = p_func
	_command_descriptions[_name] = p_desc


## Unregisters the command specified by its name or a callable.
func unregister_command(p_func_or_name: Variant) -> void:
	var cmd_name: String
	if p_func_or_name is Callable:
		var key: Variant = _commands.find_key(p_func_or_name)
		if key != null:
			cmd_name = key
	elif p_func_or_name is String:
		cmd_name = p_func_or_name
	if cmd_name.is_empty() or not _commands.has(cmd_name):
		push_error("Console: Unregister failed - command not found: " % [p_func_or_name])
		return
...
func add_argument_autocomplete_source(p_command: String, p_argument: int, p_source: Callable) -> void:
	if not p_source.is_valid():
		push_error("Console: Can't add autocomplete source: source callable is not valid")
		return
	if not has_command(p_command):
		push_error("Console: Can't add autocomplete source: command doesn't exist: ", p_command)
		return
	if p_argument < 0 or p_argument > 4:
		push_error("Console: Can't add autocomplete source: argument index out of bounds: ", p_argument)
		return
	var argument_values: Variant = p_source.call()
	if not _validate_autocomplete_result(argument_values, p_command):
		push_error("Console: Failed to add argument autocomplete source: Callable must return an array.")
		return
...
func execute_command(p_command_line: String, p_silent: bool = false) -> void:
	...
	if valid:
		var err: Variant = cmd.callv(command_args)
...
func execute_script(p_file: String, p_silent: bool = true) -> void:
	if FileAccess.file_exists(p_file):
		if not p_silent:
			Console.info("Executing " + p_file);
		var fa := FileAccess.open(p_file, FileAccess.READ)
		while not fa.eof_reached():
			var line: String = fa.get_line()
			Console.execute_command(line, p_silent)
	else:
		Console.error("File not found: " + p_file.trim_prefix("user://"))
...
func usage(p_command: String) -> Error:
	...
	for i in range(method_info.args.size() - callable.get_bound_arguments_count()):
		...
		if i >= num_required_args:
			var def_value: Variant = method_info.default_args[i - num_required_args]
		...
		if _argument_autocomplete_sources.has([p_command, i]):
			...
			var arg_autocompletes: Variant = auto_complete_callable.call()
...
func add_eval_input(p_name: String, p_value: Variant) -> void:
	_eval_inputs[p_name] = p_value


## Remove specified input variable from "eval" command.
func remove_eval_input(p_name: Variant) -> void:
...
func _add_aliases_from_config() -> void:
	for alias: String in _options.aliases:
		var target: Variant = _options.aliases[alias]
		if not alias is String:
			push_error("Console: Config error: Alias name should be String")
		elif not target is String:
			push_error("Console: Config error: Alias target should be String")
		elif has_command(alias):
			push_error("Console: Config error: Alias or command already registered: ", alias)
		elif not has_command(target):
			push_error("Console: Config error: Alias target not found: ", target)
		else:
...
func _parse_command_line(p_line: String) -> PackedStringArray:
	...
	for _char in line:
		if _char == '"':
			...
			in_brackets = false
		elif _char == ' ' and not in_quotes and not in_brackets:
			if cur > start:
...
func _expand_alias(p_argv: PackedStringArray) -> PackedStringArray:
	...
	if current_depth >= max_depth:
		push_error("Console: Max depth for alias reached. Is there a loop in your aliasing?")
		return p_argv
	return result
...
func _parse_argv(p_argv: PackedStringArray, p_callable: Callable, r_args: Array) -> bool:
	...
	for i in range(1, p_argv.size()):
		...
		if expected_type == TYPE_STRING:
			...
			var vec: Variant = _parse_vector_arg(a)
			if vec != null:
				r_args[i - 1] = vec
			else:
				r_args[i - 1] = a
...
func _parse_vector_arg(p_text: Variant) -> Variant:
	...
	if comp.size() == 2:
		...
		error("Console supports 2,3,4-element vectors, but %d-element vector given." % [comp.size()])
		return null
...
func _reverse_autocomplete() -> void:
	if not _autocomplete_matches.is_empty():
		var match_str: String = _autocomplete_matches[_autocomplete_matches.size() - 1]
...
func _add_argument_autocompletes(p_argv: PackedStringArray) -> void:
	...
	if _argument_autocomplete_sources.has(key):
		var argument_values: Variant = _argument_autocomplete_sources[key].call()
		if not _validate_autocomplete_result(argument_values, command):
			argument_values = []
		var matches: PackedStringArray = []
		for value: Variant in argument_values:
...
func _validate_callable(p_callable: Callable) -> bool:
	...
	if p_callable.is_standard() and method_info.is_empty():
		push_error("Console: Couldn't find method info for: " + p_callable.get_method())
		return false
	if p_callable.is_custom() and not method_info.is_empty() \
		...
		and p_callable.get_bound_arguments_count() > 0:
			push_error("Console: bound anonymous functions are unsupported")
			return false

	var ret := true
	for arg: Variant in method_info.args:
		if not arg.type in [TYPE_NIL, TYPE_BOOL, TYPE_INT, TYPE_FLOAT, TYPE_STRING, TYPE_VECTOR2, TYPE_VECTOR2I, TYPE_VECTOR3, TYPE_VECTOR3I, TYPE_VECTOR4, TYPE_VECTOR4I]:
			push_error("Console: Unsupported argument type: %s is %s" % [arg.name, type_string(arg.type)])
			ret = false
	return ret
...
func _validate_autocomplete_result(p_result: Variant, p_command: String) -> bool:
	if typeof(p_result) < TYPE_ARRAY:
		push_error("Console: Argument autocomplete source failed: Expecting array but got: ",
...
func log_warning(message: String) -> void:
	...
	if Debugger:
		Debugger.log_debug("[WARNING] " + message)

...
func _ready() -> void:
	...
	v_scroll.visibility_changed.connect(_hide_scrollbars)
	h_scroll.visibility_changed.connect(_hide_scrollbars)
	_hide_scrollbars()
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.is_pressed():
		match event.keycode:
			...
			KEY_C:
				if event.get_modifiers_mask() == KEY_MASK_CTRL and get_selected_text().is_empty():
					text = ""
					text_changed.emit()
					get_viewport().set_input_as_handled()
			KEY_RIGHT, KEY_END:
				if get_caret_column() == text.length() and not autocomplete_hint.is_empty():
...
func _draw() -> void:
	...
	draw_string(
		_font, 
		Vector2(offset_x, offset_y), autocomplete_hint, 
		HORIZONTAL_ALIGNMENT_LEFT, -1, 
		_font_size, _hint_color
		)
		...
		for _char : String in text + ' ':
			if _char == ' ':
				if cur > start:
					...
					if Console.has_command(maybe_command) or Console.has_alias(maybe_command):
						command_end_idx = cur
				start = cur + 1
			cur += 1
